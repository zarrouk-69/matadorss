...
if(isset($_POST['forminscription'])) {
   ...
   $mail = htmlspecialchars($_POST['adresseU']);
   $mail2 = htmlspecialchars($_POST['mail2']);
   $mdp = sha1($_POST['mdp']);
   $mdp2 = sha1($_POST['mdp2']);
   if(!empty($_POST['nomU']) AND !empty($_POST['adresseU']) AND !empty($_POST['mail2']) AND !empty($_POST['mdp']) AND !empty($_POST['mdp2'])) {
      $nomUlength = strlen($nomU);
      if($nomUlength <= 255) {
         if($mail == $mail2) {
            if(filter_var($mail, FILTER_VALIDATE_EMAIL)) {
               $reqmail = $bdd->prepare("SELECT * FROM utilisateur WHERE adresseU = ?");
               ...
               if($mailexist == 0) {
                  if($mdp == $mdp2) {
                     ...
                     $insertmbr = $bdd->prepare("INSERT INTO utilisateur (nomU,adresseU, mdpU, confirmkey) VALUES(?, ?, ?, ?)");
 
                     $insertmbr->execute(array($nomU, $mail, $mdp, $key));
         ...
         $erreur = "Votre nomU ne doit pas dépasser 255 caractères !";
      }
   } else {
...
window.onclick = function(event) {
    ...
    <section class="service-wrapper py-3">
        <div class="container-fluid pb-3">
            <div class="row">
                <h2 class="h2 text-center col-12 py-5 semi-bold-600">inscription</h2>
         ...
         <form method="POST" action="">
            <table border="0" cellpadding="10" cellspacing="10" width="500" align="center" class="tblLogin">
               ...
               <tr>
                  ...
                  <td>
                     <input type="text" placeholder="Votre mail" id="mail" name="adresseU" value="<?php if(isset($mail)) { echo $mail; } ?>" />
                  </td>
               </tr>
               <tr>
                  <td align="right">
